...
################## get file names/dir #########################
###############################################################
label_dir = "receptor-background-removed/"
file_general_name = 'pos13-T_'
appendix = "_ch_2"
output_dir = 'Meshes-Podocalyxin/'
...
t = [int(re.search(file_general_name + '(.*)' + appendix, fn).group(1))   for fn in mask_filenames ]
...
def GoThroughAllFiles(t, fn, label_dir):
		...
		for i, mask_id in enumerate(ids):
			...
			smooth.save(output_dir + str(t).zfill(3)+"/T_" + str(t).zfill(3) + "_podmesh_"+str(mask_id).zfill(3)+".stl")
...
for T, fn in zip(t[i * Nsteps : (i+1) * Nsteps],mask_filenames[i * Nsteps : (i+1) * Nsteps]):
	GoThroughAllFiles(T, fn, label_dir)
