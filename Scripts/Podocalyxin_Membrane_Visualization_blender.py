...
Membrane_mat = bpy.data.materials["Membrane"]
...
for t in all_T: 
    ...
    for fn in filenames:
        ...
        file_general_name ='T_' + folder_t + "_podmesh_"
        appendix = ".stl"
        print(file_general_name + '(.*)' + appendix)
...
all_mem_meshes = 'Meshes-Membrane/'

all_T = sorted([int(os.path.basename(fn)) for fn in glob.glob(all_mem_meshes + '*')])
...
for t in all_T: 
    ...
    folder_t = str(t).zfill(3)
    filepath = all_mem_meshes + folder_t + '/'
    ...
    for fn in filenames:
        ...
        ob.data.materials.append(Membrane_mat)
